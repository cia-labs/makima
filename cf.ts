...
export async function getEmailRoutingAddresses(): Promise<AxiosResponse<EmailRoutingAddressesResponse>> {
    ...
    try {
        const response = await axios.get<EmailRoutingAddressesResponse>(url, { headers });
        return response;
    } catch (error) {
        throw error;
    }
...
export async function createEmailRoutingAddress(email: string): Promise<AxiosResponse<CreateEmailRoutingAddressResponse>> {
    const url = `https://api.cloudflare.com/client/v4/accounts/${accountIdentifier}/email/routing/addresses`;
    ...
    try {
        const response = await axios.post<CreateEmailRoutingAddressResponse>(url, data, { headers });
        return response;
    } catch (error) {
        throw error;
    }
...
export async function createCialabsEmail(
    ...
    try {
      const response = await axios.post<CreateCialabsEmailRuleResponse>(url, ruleData, { headers });
      return response;
    } catch (error) {
      throw error;
    }
